...
mod tests {
    ...
    async fn stream_exceeding_max_bytes_is_rejected_and_partial_removed() {
        ...
        let chunks: Vec<reqwest::Result<bytes::Bytes>> = (0..3)
            .map(|_| Ok(bytes::Bytes::from(vec![0u8; 100])))
            .collect();
        let res = stream_verify_adopt(
        ...
        assert!(
            !partial.exists(),
            "partial must be removed after the cap trips"
        );
    }
