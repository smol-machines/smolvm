...
use crate::internal::platform::Os;
...
pub(super) fn find_e2fsprogs_tool(name: &str) -> Option<String> {
...
pub(super) fn create_sparse_disk<D: DiskType>(path: &Path, size_bytes: u64) -> Result<()> {
...
pub(super) fn copy_disk_from_template<D: DiskType>(
    disk_path: &Path,
    size_bytes: u64,
    template_path: &Path,
) -> Result<()> {
...
pub(super) fn expand_sparse_disk<D: DiskType>(path: &Path, new_size_gib: u64) -> Result<()> {
    use std::fs::OpenOptions;

    let new_size_bytes = new_size_gib * BYTES_PER_GIB;
...
pub(super) fn format_disk_with_mkfs<D: DiskType>(disk_path: &Path) -> Result<()> {
    tracing::info!(
        path = %disk_path.display(),
        disk_type = D::NAME,
...
pub(super) fn clone_or_copy_file(src: &Path, dst: &Path) -> Result<()> {
    #[cfg(target_os = "macos")]
    {
        use std::ffi::CString;
