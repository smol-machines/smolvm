...
test_microvm_resize_happy_path() {
    ...
    $SMOLVM microvm create "$vm_name" --storage 5 --overlay 2 2>&1 || return 1
    ...
    # Resize
    local resize_output
    resize_output=$($SMOLVM microvm resize "$vm_name" --storage 10 --overlay 5 2>&1)

    # Verify resize output contains expected messages
    if [[ "$resize_output" != *"Resizing microvm"* ]]; then
        echo "Resize output should contain 'Resizing microvm'"
        $SMOLVM microvm delete "$vm_name" -f 2>/dev/null
        rm -rf "$(vm_data_dir "$vm_name")"
        return 1
    fi
...
test_microvm_resize_running_vm_rejected() {
    ...
    local exit_code=0
    local resize_output
    resize_output=$($SMOLVM microvm resize "$vm_name" --storage 10 2>&1) || exit_code=$?
...
test_microvm_resize_shrink_rejected() {
    ...
    $SMOLVM microvm create "$vm_name" --storage 10 --overlay 5 2>&1 || return 1
    $SMOLVM microvm start "$vm_name" 2>&1 || {
        $SMOLVM microvm delete "$vm_name" -f 2>/dev/null
        return 1
    }
    ...
    local exit_code=0
    local resize_output
    resize_output=$($SMOLVM microvm resize "$vm_name" --storage 5 2>&1) || exit_code=$?
...
test_microvm_resize_nonexistent_vm_rejected() {
...
test_microvm_resize_default_vm() {
    ...
    local resize_output
    resize_output=$($SMOLVM microvm resize --storage 30 --overlay 15 2>&1)
...
test_microvm_resize_no_params_rejected() {
    ...
    local exit_code=0
    local resize_output
    resize_output=$($SMOLVM microvm resize "$vm_name" 2>&1) || exit_code=$?
...
test_microvm_resize_storage_only() {
    ...
    # Resize storage only
    local resize_output
    resize_output=$($SMOLVM microvm resize "$vm_name" --storage 15 2>&1)
...
test_microvm_resize_overlay_only() {
    ...
    # Resize overlay only
    local resize_output
    resize_output=$($SMOLVM microvm resize "$vm_name" --overlay 8 2>&1)
...
run_test "Resize: happy path" test_microvm_resize_happy_path || true
run_test "Resize: running VM rejected" test_microvm_resize_running_vm_rejected || true
run_test "Resize: shrink rejected" test_microvm_resize_shrink_rejected || true
run_test "Resize: non-existent VM rejected" test_microvm_resize_nonexistent_vm_rejected || true
run_test "Resize: default VM" test_microvm_resize_default_vm || true
run_test "Resize: no params rejected" test_microvm_resize_no_params_rejected || true
run_test "Resize: storage only" test_microvm_resize_storage_only || true
run_test "Resize: overlay only" test_microvm_resize_overlay_only || true
